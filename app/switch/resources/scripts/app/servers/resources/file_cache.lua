...
--include local.lua if it exists
local localconfig = {}
local f,err = loadfile("/etc/fusionpbx/local.lua", "t", localconfig)
if f then
		f()
else
		freeswitch.consoleLog("NOTICE","file_cache.lua: failed to load local.lua\n");
		return
end
...
--send the API commands
	...
	for key,row in pairs(localconfig.servers) do
