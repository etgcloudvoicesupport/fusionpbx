...
local forward_no_answer_destination = session:getVariable("forward_no_answer_destination") or '';
local extension;
...
local forward_no_answer_enabled = row.forward_no_answer_enabled;
local last_forward_no_answer_destination = row.forward_no_answer_destination;
...
if enabled == "toggle" then
    ...
    if (#forward_no_answer_destination == 0) or (forward_no_answer_destination == row.forward_no_answer_destination) then
        enabled = (forward_no_answer_enabled == "true") and "false" or "true";
    else
        enabled = 'true'
    end
end
...
if enabled == 'true' and #forward_no_answer_enabled == 0 then
...
if enabled == "true" and destination_by_arg then
    forward_no_answer_destination = destination_by_arg;
end

--get the forward destination by IVR if destination has not been passed by argument
if enabled == "true" and empty(forward_no_answer_destination) and not destination_by_arg then
    forward_no_answer_destination = session:playAndGetDigits(min_digits, max_digits, max_tries, digit_timeout, "#", sounds_dir.."/"..default_language.."/"..default_dialect.."/"..default_voice.."/ivr/ivr-enter_destination_telephone_number.wav", "", "\\d+");
    if empty(forward_no_answer_destination) then return end
end
...
if enabled == "true" then
    ...
    --say the destination number
        session:say(forward_no_answer_destination, default_language, "number", "iterated");
    --notify the caller
...
if empty(forward_no_answer_destination) then
    enabled = "false";
    forward_no_answer_enabled = "false";
end
...
do
    ...
    sql = sql .. "forward_no_answer_enabled = :forward_no_answer_enabled ";
    sql = sql .. "where domain_uuid = :domain_uuid ";
    sql = sql .. "and extension_uuid = :extension_uuid ";
    local params = {
...
if (activated == "t") then
    local ringotel_data = {method = "setUserSettings", params = {id = ringotel_id, orgid = orgid, fwdnans = true, fwdnansnumber = forward_no_answer_destination}};
    local fwd_result = ringotel_api.post(json.encode(ringotel_data));
    freeswitch.consoleLog("notice", "[feature_event] ringotel api result: " .. fwd_result .. "\n");
end
...
if settings:get('device', 'feature_sync', 'boolean') == 'true' then
    ...
    if (sip_profiles ~= nil) then
            freeswitch.consoleLog("NOTICE", "[feature_event] SIP NOTIFY: CFWDNOANSWER set to "..forward_no_answer_enabled.."\n");
...
--     last_forward_no_answer_destination, forward_no_answer_destination, domain_name);
