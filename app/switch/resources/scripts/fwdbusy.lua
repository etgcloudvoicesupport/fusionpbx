...
local forward_busy_destination = session:getVariable("forward_busy_destination") or '';
local extension;
...
local forward_busy_enabled = row.forward_busy_enabled;
local last_forward_busy_destination = row.forward_busy_destination;
...
if enabled == "toggle" then
    ...
    if (#forward_busy_destination == 0) or (forward_busy_destination == row.forward_busy_destination) then
        enabled = (forward_busy_enabled == "true") and "false" or "true";
    else
        enabled = 'true'
    end
end
...
if enabled == 'true' and #forward_busy_enabled == 0 then
...
if enabled == "true" and destination_by_arg then
    forward_busy_destination = destination_by_arg;
end

--get the forward destination by IVR if destination has not been passed by argument
if enabled == "true" and empty(forward_busy_destination) and not destination_by_arg then
    forward_busy_destination = session:playAndGetDigits(min_digits, max_digits, max_tries, digit_timeout, "#", sounds_dir.."/"..default_language.."/"..default_dialect.."/"..default_voice.."/ivr/ivr-enter_destination_telephone_number.wav", "", "\\d+");
    if empty(forward_busy_destination) then return end
end

--set call forward
if enabled == "true" then
    --set forward_busy_enabled
        forward_busy_enabled = "true";
        channel_display(session:get_uuid(), "Activated")
    --say the destination number
        session:say(forward_busy_destination, default_language, "number", "iterated");
    --notify the caller
...
if empty(forward_busy_destination) then
    enabled = "false";
    forward_busy_enabled = "false";
end
...
do
    ...
    sql = sql .. "forward_busy_enabled = :forward_busy_enabled ";
    sql = sql .. "where domain_uuid = :domain_uuid ";
    sql = sql .. "and extension_uuid = :extension_uuid ";
    local params = {
...
--update ringotel API
    ...
    if (activated == "t") then
        local ringotel_data = {method = "setUserSettings", params = {id = ringotel_id, orgid = orgid, fwdbusy = (forward_busy_enabled == "true"), fwdbusynumber = forward_busy_destination}};
...
if settings:get('device', 'feature_sync', 'boolean') == 'true' then
    ...
    if (sip_profiles ~= nil) then
            freeswitch.consoleLog("NOTICE", "[feature_event] SIP NOTIFY: CFWDBUSY set to "..forward_busy_enabled.."\n");
